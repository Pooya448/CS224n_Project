...
def negSamplingLossAndGradient(
    ...
    term_U_ks = sigmoid(-1 * np.dot(U_ks, centerWordVec))
    term_U_o = sigmoid(np.dot(U_o, centerWordVec))

    loss = -1 * np.log(term_U_o) - np.sum(np.log(term_U_ks))

    gradCenterVec += np.dot(U_ks.T, (1 - term_U_ks)) + U_o * (1.0 - term_U_o) * -1
    gradOutsideVecs[outsideWordIdx] += (1 - term_U_o) * centerWordVec * -1

    for negIdx in negSampleWordIndices:
        gradOutsideVecs[negIdx] += (1 - sigmoid(np.dot(-1 * outsideVectors[negIdx], centerWordVec))) * centerWordVec
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        currentLoss, currentGradCenterVec, currentGradOutsideVecs = word2vecLossAndGradient(currentCenterWordVector, outsideWordIdx, outsideVectors, dataset)
